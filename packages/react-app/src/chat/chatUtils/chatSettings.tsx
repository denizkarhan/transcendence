...
const ChatSettings = (props: Props) => {
	...
	const handleBlockUser = async (username:string) => {
		const response = await api.get(`/block-user/${username}`);
	...
	const handleMute = (username:string, roomName:string) =>{
							...
							{props.users.map(user => (
								<div key={user.users.Login} style={{
									...
									<Stack key={user.users.Login} direction="horizontal" gap={2}>
										<Button onClick={() => handleBlockUser(user.users.Login)} bsPrefix="btn btn-outline-info"><i className="bi bi-slash-circle"></i></Button> {/*block user */}
										<Button onClick={() => handleKick(user.users.Login)} bsPrefix="btn btn-outline-info"><i className="bi bi-box-arrow-right"></i></Button>{/*kick user */}
										<Button bsPrefix="btn btn-outline-info"><i className="bi bi-volume-mute"></i></Button>{/*mute user */}
									</Stack>
								</div>
							))}
						</Tab>
